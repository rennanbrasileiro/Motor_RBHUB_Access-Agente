...
export function parseTopAccessExportLine(line: string, source = "topaccess-export"): NormalizedAccessEvent | null {
  ...
  let eventType: NormalizedAccessEvent["eventType"] = "unknown";
  ...
  if (nameStart >= 0 && nameStart + 1 < tokens.length) {
    ...
    if (/^(E|S|NEG)$/i.test(employeeName)) employeeName = undefined;
  }
